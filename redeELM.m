...
for i=1:17 %para todos os testes
    ...
    str = ['Acuracia obtida com todas as variaveis: ' num2str(acuracia(i)) ' %'];
...
%Aplicando ELM apenas com as duas caracteristicas mais relevantes
%separa o dataset em treino e teste

Xa(1,:) = X(1,:); %Caracteristica 1
Xa(2,:) = X(7,:); %Caracteristica 7
for i=1:17 % para todos os testes
    XTreinamento = Xa; 
    YTreinamento = Y;
    clear XTeste;
    for j=5:-1:1 % cinco caracteristicas testam o modelo
        XTeste(:,j) = Xa(:,(i-1)*5 + j); %amostras para testar o modelo
        XTreinamento(:,(i-1)*5 + j) = []; %ficam so as amostras que vão treinar o modelo
        YTeste(:,j) = Y(:,(i-1)*5 + j); %classes para verificar acuracia do modelo
        YTreinamento(:,(i-1)*5 + j) = []; %ficam so as classes que vão treinar o modelo
    end
...
str = ['Acuracia media obtida com apenas as duas variaveis mais descritivas: ' num2str(mean(acuracia)) ' %'];
disp(str); %exibe a Acuracia obtida